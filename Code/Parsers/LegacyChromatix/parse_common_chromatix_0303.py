# -*- coding: utf-8 -*-
"""
Chromatix VFE common parser 0x0303 (early revision, chroma*_0303.h headers).
...
chromatix_linearization_type contains for R/GR/GB/B:
    ...
    *_lut_base[9] (u16),
    *_lut_delta[9] (float).
Delta is no longer present in newer versions, but it is still present in 0303. :contentReference[oaicite:10]{index=10}
...
# chromatix_linearization_type (0303-style):
#   r_lut_p[8] (u16)
#   r_lut_base[9] (u16)
#   r_lut_delta[9] (float)
...
# then six linear_table_*_* tables (chromatix_linearization_type). :contentReference[oaicite:18]{index=18}
...
# mesh_rolloff_array_type (0303-style):
#   mesh_rolloff_table_size (u16),
...
# chromatix_rolloff_type (0303-style):
...
# chromatix_VFE_common_type (0303-style) top-level: :contentReference[oaicite:22]{index=22}
...
#   chromatix_rolloff      (chromatix_rolloff_type)
#   chromatix_LA_special_effects (chromatix_LA_special_effects_type)
chromatix_VFE_common_type = StructSpec("chromatix_VFE_common_type_0303", [
...
def parse_chromatix_common(
    ...
    """
    Build CSV strings for chromatix_VFE_common_type (version 0303).
